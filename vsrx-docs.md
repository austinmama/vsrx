...
[Enabling on-box security event reporting ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://public.dhe.ibm.com/cloud/bluemix/network/vsrx/on-box-logging-reporting-11320.pdf){:new_window} | During IBM Cloud deployment and provisioning of the vSRX, basic settings for the gateway are set, which do not include the on-box reporting configuration. You can enable this feature by configuring the `set security log report` CLI command at the `edit security log` hierarchy.
